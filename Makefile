# This Makefile is for the Tie::Google extension to perl.
#
...
#	NAME => q[Tie::Google]
#	PREREQ_PM => { Net::Google=>undef, Symbol=>undef }
#	VERSION => q[0.01]
#	clean => { FILES=>q[$(DISTVNAME).tar$(SUFFIX) *.xsi .googlekey] }
#	macro => { GOOGLE_KEY=>q[] }
...
TO_INST_PM = Google.pm

PM_TO_BLIB = Google.pm \
	$(INST_LIBDIR)/Google.pm
...
manifypods : pure_all Google.pm
	@$(POD2MAN) \
	Google.pm \
	$(INST_MAN3DIR)/Tie::Google.$(MAN3EXT)
...
clean ::
	-rm -rf $(DISTVNAME).tar$(SUFFIX) *.xsi .googlekey ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c mon.out core core.*perl.*.? *perl.core so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def $(BASEEXT).exp
...
ppd:
	@$(PERL) -e "print qq{<SOFTPKG NAME=\"Tie-Google\" VERSION=\"0,01,0,0\">\n}. qq{\t<TITLE>Tie-Google</TITLE>\n}. qq{\t<ABSTRACT></ABSTRACT>\n}. qq{\t<AUTHOR></AUTHOR>\n}. qq{\t<IMPLEMENTATION>\n}. qq{\t\t<DEPENDENCY NAME=\"Net-Google\" VERSION=\"0,0,0,0\" />\n}. qq{\t\t<DEPENDENCY NAME=\"Symbol\" VERSION=\"0,0,0,0\" />\n}. qq{\t\t<OS NAME=\"$(OSNAME)\" />\n}. qq{\t\t<ARCHITECTURE NAME=\"i686-linux\" />\n}. qq{\t\t<CODEBASE HREF=\"\" />\n}. qq{\t</IMPLEMENTATION>\n}. qq{</SOFTPKG>\n}" > Tie-Google.ppd
